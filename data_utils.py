...
class SentenceDataset(Dataset): 
    def __init__(self, fname, tokenizer, opt, vocab_help): 
        ...
        for obj in tqdm(parse(fname), total=len(parse(fname)), desc="Training examples"): 
            ...
            post = [p + opt.max_position + 1 for p in post]
...
class Tokenizer4BertGCN:
    def __init__(self, max_seq_len, pretrained_roberta_name=None):
        ...
        self.cls_token_id = self.tokenizer.convert_tokens_to_ids("<s>")
...
class ABSAGCNData(Dataset):
    def __init__(self, fname, tokenizer, pos_vocab, dep_vocab, opt): 
        self.data = []
        parse = ParseData
        examples = list(parse(fname))
        polarity_dict = {'positive':0, 'negative':1, 'neutral':2} 
        pad_id = getattr(tokenizer, 'pad', 0)
        ...
        for obj in examples: 
            ...
            context_asp_ids = [tokenizer.cls_token_id] + tokenizer.convert_tokens_to_ids(bert_tokens) + [tokenizer.sep_token_id] + tokenizer.convert_tokens_to_ids(term_tokens) + [tokenizer.sep_token_id]
            context_asp_len = len(context_asp_ids)
            paddings = [pad_id] * (tokenizer.max_seq_len - context_asp_len)

            context_len = len(bert_tokens)
            context_asp_seg_ids = [0] * (context_len + 2) + [1] * (len(term_tokens) + 1) + paddings
            ...
            context_asp_attention_mask = [1] * context_asp_len + [0] * len(paddings)
            ...
            data = {
                ...
                'adj_dep': context_asp_adj_matrix,
                'src_mask': src_mask,
